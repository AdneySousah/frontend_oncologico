...
export default function MotivosFalhaContato() {
  ...
  const fetchMotivos = async () => {
    try {
      setLoading(true);
      const response = await api.get('/motivos-falha-contato?all=true');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await api.put(`/motivos-falha-contato/${editingId}`, payload);
        toast.success('Motivo atualizado com sucesso!');
      } else {
        await api.post('/motivos-falha-contato', payload);
  ...
  const handleToggleAtivo = async (id, statusAtual) => {
    try {
      setLoading(true);
      await api.put(`/motivos-falha-contato/${id}`, { ativo: !statusAtual });
  ...
  return (
    <Container>
      <Header>
        <h2>Gerenciar Motivos de Falha de Contato</h2>
          ...
          <ModalContent>
            ...
            <form onSubmit={handleSubmit}>
              <FormGroup>
                ...
                <Input 
                  type="text"
                  placeholder="Ex: Paciente não atendeu"
                  value={descricao}
