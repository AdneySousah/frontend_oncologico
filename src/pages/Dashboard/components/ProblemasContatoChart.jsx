...
const ProblemasContatoChart = ({ chartData, reportData }) => {
  const handleExport = () => {
    const columns = [
      { header: 'ID Paciente', key: 'paciente_id', width: 15 },
      { header: 'Nome Paciente', key: 'nome_paciente', width: 35 },
      { header: 'Motivo da Falha', key: 'motivo_falha', width: 40 },
      { header: 'Data Registro', key: 'data_registro', width: 20 },
      { header: 'Operadora', key: 'operadora', width: 20 },
    ];
    exportToXLSX(reportData, columns, 'Relatorio_Problemas_Contato', 'Falhas de Contato - Pacientes por Motivo');
  };

  return (
    <>
      <ChartHeader>
        <h3>Problemas de Contato (Motivos de Falha)</h3>
      ...
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={chartData} margin={{ top: 20, right: 10, left: -20, bottom: 0 }}>
          ...
          <Bar
            dataKey="value"
            fill="#F97316" /* Laranja (diferente da ficha RAM para contraste) */
            radius={[6, 6, 0, 0]}
            maxBarSize={60}
          >
...
export default memo(ProblemasContatoChart);