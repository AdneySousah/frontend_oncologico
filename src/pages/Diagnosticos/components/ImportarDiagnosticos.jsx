...
export default function ImportarDiagnosticos({ onSuccess }) {
  ...
  const handleFileChange = async (e) => {
    ...
    if (selected) {
      ...
      try {
        const response = await api.post('/diagnosticos/validate', formData);
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      const res = await api.post('/diagnosticos/import', formData);
      toast.success(`Sucesso! ${res.data.inseridos} novos diagnósticos importados.`);
      setStep(5);
      ...
      {step === 1 && (
        <>
          <S.InstructionsBox>
            ...
            <p>O sistema verificará o nome do <strong>Diagnóstico</strong> na planilha e adicionará os não existentes.</p>
            <ul>
                <li>A planilha deve conter uma coluna chamada <strong>"diagnostico"</strong>, <strong>"Diagnostico"</strong> ou <strong>"CID"</strong>.</li>
