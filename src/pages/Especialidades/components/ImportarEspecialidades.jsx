...
export default function ImportarEspecialidades({ onSuccess }) {
  ...
  const handleFileChange = async (e) => {
    ...
    if (selected) {
      ...
      try {
        const response = await api.post('/specialities/validate', formData);
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      const res = await api.post('/specialities/import', formData);
      toast.success(`Sucesso! ${res.data.inseridos} novas especialidades importadas.`);
      setStep(5);
      ...
      {step === 1 && (
        <>
          <S.InstructionsBox>
            ...
            <p>O sistema verificará o nome da <strong>Especialidade</strong> na planilha e adicionará as não existentes.</p>
            <ul>
                <li>A planilha deve conter uma coluna chamada <strong>"nome"</strong>, <strong>"name"</strong> ou <strong>"Especialidade"</strong>.</li>
